...
async function handler(req, res) {
    ...
    try {
        if (req.method === "POST") {
            ...
            const { body  } = req.body;
            const post = await _libs_prismadb__WEBPACK_IMPORTED_MODULE_1__/* ["default"].post.create */ .Z.post.create({
                data: {
                    body,
        ...
        if (req.method === "GET") {
            ...
            if (userId && typeof userId === "string") {
                posts = await _libs_prismadb__WEBPACK_IMPORTED_MODULE_1__/* ["default"].post.findMany */ .Z.post.findMany({
                    where: {
                        userId
                    },
                    include: {
